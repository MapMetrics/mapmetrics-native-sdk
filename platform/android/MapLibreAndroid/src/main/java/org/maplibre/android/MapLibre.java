...
//  }
      @UiThread
      public static synchronized void getInstance(@NonNull Context context,
                                                  @NonNull String token,
                                                  @NonNull OnMapLibreReadyCallback callback) {
            ThreadUtils.init(context);
            ThreadUtils.checkThread(TAG);
            ...
            if (INSTANCE == null) {
                  ...
                  // Wait for session initialization before returning INSTANCE
                  initializeSessionWithToken(context, token, () -> {
                        // Initialization complete, now call the callback
                        callback.onMapLibreReady(INSTANCE);
                  });
            } else {
                  // Already initialized
                  callback.onMapLibreReady(INSTANCE);
      ...
      public static synchronized void getInstance(@NonNull Context context,
              ...
              , OnMapLibreReadyCallback callback) {
            ThreadUtils.init(context);
            ThreadUtils.checkThread(TAG);
            if (INSTANCE == null) {
                  Timber.plant();
            ...
            if (apiKey != null) {
                  ...
                  // If no token, proceed immediately
                  callback.onMapLibreReady(INSTANCE);
            }
      }
       ...
       * @param callback Optional callback to be notified when initialization is complete
       */
      public static void initializeSessionWithToken(@NonNull Context context,
                                                    @NonNull String token,
